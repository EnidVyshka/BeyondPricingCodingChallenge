...
class LISTING:
    ...
    def comparison_function(
        cls, listings_list, comparison_sign, base_price, threshold_price
    ) -> List[Dict]:
        filtered_element = []
        for listing_item in listings_list:
            if comparison_sign == "gt" and base_price > threshold_price:
                filtered_element.append(listing_item)
            elif comparison_sign == "gte" and base_price >= threshold_price:
                filtered_element.append(listing_item)
            elif comparison_sign == "lt" and base_price < threshold_price:
                filtered_element.append(listing_item)
            elif comparison_sign == "lte" and base_price <= threshold_price:
                filtered_element.append(listing_item)
            elif comparison_sign == "e" and base_price == threshold_price:
                filtered_element.append(listing_item)
